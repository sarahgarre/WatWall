...
## QUALIY CHECK
    ...
    std_HUM4 = sqrt((1 / N) * SCE_HUM4 )  # calcul du std (écart type)
    std_HUM5 = sqrt((1 / N) * SCE_HUM5)   # calcul du std (écart type)
    std_HUM6 = sqrt((1 / N) * SCE_HUM6)   # calcul du std (écart type)
    ...
    std_Rn =  sqrt((1 / N) * SCE_Rn )  # calcul du std (écart type)
    std_Thr = sqrt((1 / N) * SCE_Thr )
    std_u2 =  sqrt((1 / N) * SCE_u2 )
    std_P =   sqrt((1 / N) * SCE_P )

    if std_HUM4 < LIM_HUM4 and std_HUM5 < LIM_HUM5 and std_HUM6 < LIM_HUM6   # Quality check for all 3 WC sensor OK
        if VWC < Water_Content_Limit_To_Set                                  # Check whether irrigation is needed

        if std_Rn < LIM_Rn and std_Thr < LIM_Thr and std_u2 < LIM_u2 and std_P < LIM_P  # Quality check for all 3 WC sensor OK
